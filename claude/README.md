# Claude Code skill: `jira-multi`

`skills/jira-multi/SKILL.md` teaches Claude Code how to use the
`jira-multi-mcp` server well — calling `jira_sites` first, omitting `site`
...
cp -R claude/skills/jira-multi ~/.claude/skills/jira-multi
```

Or, for a single project, into that project's `.claude/skills/` instead of
the global one. Either way, `jira-multi-mcp` itself must also be registered
