...
`JIRA_MULTI_TEST_SANDBOX_ISSUE` is set -- see the module-level skip below.
...
pytestmark = [
    ...
    pytest.mark.skipif(
        os.environ.get("JIRA_MULTI_INTEGRATION") != "1",
        reason="set JIRA_MULTI_INTEGRATION=1 to run real-network integration tests",
    ),
...
def _sandbox_issue_key() -> str | None:
    return os.environ.get("JIRA_MULTI_TEST_SANDBOX_ISSUE")
...
async def test_get_issue_and_list_attachments_on_every_site(
    ...
    for site in real_config.sites:
        ...
        assert get_result.meta["jira-multi-mcp/site"] == site.name
...
@pytest.mark.skipif(
    ...
    reason="set JIRA_MULTI_TEST_SANDBOX_ISSUE to a writable issue key to run the attachment write round trip",
)
async def test_attachment_upload_list_download_delete_round_trip(
    ...
    if issue_key is None:
        ...
        raise RuntimeError(
            "refusing to run the attachment write round trip without JIRA_MULTI_TEST_SANDBOX_ISSUE set"
        )
    ...
    content = f"jira-multi-mcp integration test round trip for {issue_key}\n".encode()
    upload_path = tmp_path / "jira-multi-mcp-test-upload.txt"
